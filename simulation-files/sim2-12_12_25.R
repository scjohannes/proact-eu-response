...
available_effect_sizes <- available_effect_sizes[c(3,5,7)]
...
fit_markov_boot <- function(data, iter) {
  ...
  boot_coefs <- tidy_bootstrap_coefs(boot_output[["coefs"]],
                                     probs = c(0.024, 0.976),  # use alpha = 0.024
                                     estimate = "median") |>
    filter(term == "tx") |>
    select(!n_iter)
  ...
  boot_results <- cbind(boot_tis = boot_tis, boot_coefs = boot_output[["coefs"]])
  naive_vglm <- summary(fit)@coef3["tx", c("Estimate", "Pr(>|z|)")]
...
saveRDS(final_seed, "seed_after_initial_run12_12_25.rds")

# Add this at the beginning of the document in future extensions of these simulations
#.Random.seed <- readRDS("seed_after_initial_run12_12_25.rds")
